"""Train the MLP under the *exact same* optimization setting as test_train_cnn.py,
so that the MLP-vs-CNN comparison varies only one factor: the model architecture.

Pair this with `python test_train_cnn.py` (default args). The two runs share:
    optimizer = MomentGD(lr=0.05, mu=0.9)
    scheduler = MultiStepLR(milestones=[total/2, total*3/4], gamma=0.5)
    batch_size = 64
    epochs = 3
    weight_decay (lambda) = 1e-4 on every trainable layer
    data split = same idx.pickle, same /255 normalisation
"""
import os
import argparse
import mynn as nn
...
parser.add_argument('--save-tag', type=str, default='mlp_fair')
...
# Reuse the same split as MLP / CNN runs so all three histories live in the same world.
...
# Same MLP shape as test_train.py (784->600->10) and same weight-decay lambdas
# so the only difference vs CNN is the model architecture itself.
...
runner.train([train_imgs, train_labs], [valid_imgs, valid_labs],
             num_epochs=args.epochs, log_iters=100, eval_iters=100, save_dir=save_dir)
